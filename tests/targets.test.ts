...
describe('порядок тел в списке', () => {
  it('соответствует прежнему перечислению вручную', () => {
    ...
    expect(listOrder()).toEqual([
      ...
      'pluto',
      'charon',
      // Комета - за Плутоном, и это решение, а не случайность: порядок в
      // списке по удалению от Солнца, а у Галлеи расстояние меняется в
      // шестьдесят раз, и места среди планет у неё нет.
      'halley',
    ]);
  });
...
describe('род тела', () => {
  it('знает исключения', () => {
    ...
    // Комета не планета, и карточка обязана это знать: род берётся из
    // отдельной таблицы, а не из того, в каком списке тело лежит.
    expect(kindOf('halley')).toBe('комета');
