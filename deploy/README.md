...
Тег ставится именно `git tag`, а не `gh release create`. Последний создаёт тег
сам, попутно создавая и релиз, — и тогда workflow, запущенный этим же тегом,
застаёт релиз уже готовым. Раньше он на этом падал, а вместе с ним не
состоялась и выкладка: на сервере оставалась прежняя версия при зелёном на вид
теге. Теперь шаг релиза это переживает — если релиз уже есть, архив сборки
просто догружается на него, — но порядок всё равно такой: тег, а релиз создаёт
CI.

Сорвавшийся выпуск переигрывается без перестановки тега:

```bash
gh workflow run release.yml -f tag=v0.4.0
```

Событие push для тега случается один раз, поэтому у workflow есть ручной вход.
Он собирает и выкладывает ровно тот коммит, на котором стоит указанный тег.
