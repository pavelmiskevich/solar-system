...
export default defineConfig({
  ...
  // Две минуты, а не полторы: время здесь уходит не на логику проверки, а на
  // кадры, и на загруженном runner тот же перелёт растягивается вдвое против
  // обычного. Проверка, которой не хватило срока, падает не по делу.
  timeout: 120_000,
  ...
  use: {
    ...
    // Язык интерфейса выбирается по браузеру, а Playwright по умолчанию
    // представляется английским. Проверки ищут кнопки и подписи по русскому
    // тексту, поэтому браузер здесь русский; английский интерфейс проверяется
    // отдельно и задаётся там явно - ссылкой или своим контекстом.
    locale: 'ru-RU',
