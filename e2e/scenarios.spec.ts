...
test.describe('готовые виды', () => {
  ...
  for (const [id, body] of Object.entries(EXPECTED)) {
    test(`вид «${id}» доводит камеру до тела ${body}`, async ({ page }) => {
      ...
      // Привязка к системе отсчёта тоже ждётся, а не снимается сразу: вид
      // ставит камеру, а подхватывает её кадровый цикл, и на загруженной
      // машине между одним и другим проходит больше трёх кадров. Отсюда
      // падение «targetId: null» в полном прогоне при зелёном одиночном.
      await page.waitForFunction((id) => window.sim.frame.targetId === id, body, {
        timeout: 20_000,
      });
