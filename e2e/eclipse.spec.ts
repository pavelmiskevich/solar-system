...
test.describe('затмения', () => {
  test('тень Луны падает на Землю в день затмения и не падает сутками раньше', async ({
    ...
    // А в полной тени Солнце закрыто целиком, и там просто черно. Меряется
    // здесь весь столб - и поверхность, и воздух над ней: слой рассеяния
    // однажды уже светил в тени в полную силу, и вся тень держалась на этих
    // двух числах. Порог оставлен с запасом вниз, но не вверх.
    expect(eclipsed.min).toBeLessThan(2);
