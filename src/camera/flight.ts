...
/**
 * Сколько радиан поворота даёт пиксель движения - мыши или пальца, радианы.
 *
 * Величина одна на оба устройства: палец и мышь проходят по экрану сравнимые
 * расстояния, и разводить чувствительность значило бы подгонять два числа
 * вместо одного. Протаскивание на всю высоту телефонного экрана выходит около
 * ста градусов - примерно разворот от края до края поля зрения.
 */
...
export class FlightControls {
  ...
  /**
   * Навести взгляд на точку сразу, без сглаживания.
   *
   * Нужно там, где движение камеры ведёт не мышь, а другой режим, который сам
   * это движение уже сгладил. Второй сглаживатель поверх первого не делает
   * картинку мягче - он добавляет отставание, пропорциональное скорости
   * поворота: тело съезжает с центра кадра тем дальше, чем быстрее камера
   * идёт вокруг него.
   *
   * И отставание это непостоянно. Величина шага сглаживания считается из
   * длительности кадра, а та гуляет; вслед за ней гуляет и промах, так что
   * тело дрожит вправо-влево в такт неровности кадров. При мыши это незаметно:
   * протаскивание прерывисто, и стоит его отпустить, как отставание за
   * несколько кадров сходит на нет. А в экскурсии поворот идёт непрерывно -
   * там дрожание видно прямо на глаз, и оттуда оно и было замечено.
   */
  holdAimAt(target: Vector3): void {
    this.aimAt(target);
    this.yaw = this.targetYaw;
    this.pitch = this.targetPitch;
    this.applyOrientation();
  ...
  /**
   * Повернуть взгляд протаскиванием, пиксели.
   *
   * Жест пальцем, а не захваченная мышь: на сенсорном экране захватывать
   * нечего, и осмотр там делается протаскиванием.
   *
   * Знаки обратны мышиным нарочно. Захваченная мышь двигает взгляд - повёл
   * вправо, смотришь правее. Палец двигает саму сцену: он её держит, и она
   * должна идти за ним, как идёт тело в орбитальном режиме от того же жеста.
   * Одинаковые знаки ломали бы это совпадение, и одно и то же движение пальца
   * поворачивало бы сцену в разные стороны до и после перелёта.
   */
  lookBy(dx: number, dy: number): void {
    if (!this.enabled) return;
    this.targetYaw += dx * LOOK_SENSITIVITY;
    this.targetPitch = clamp(
      this.targetPitch + dy * LOOK_SENSITIVITY,
      -Math.PI / 2 + 0.02,
      Math.PI / 2 - 0.02,
    );
  }
  ...
  private readonly onMouseMove = (e: MouseEvent): void => {
    ...
    const sensitivity = LOOK_SENSITIVITY;
