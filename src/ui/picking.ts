...
export function pickBody<T extends PickCandidate>(
   ...
   *
   * Но не всякая точка перед диском. Фобос обходит Марс в неполных трёх его
   * радиусах и почти всегда проецируется на планету, а размером он - десятая
   * доля пикселя против трёх с половиной тысяч километров. Без второго условия
   * наведение на середину Марса отдавало одиннадцатикилометровый камень, и
   * выглядело это как промах интерфейса, а не как выбор.
   *
   * Условие простое и предсказуемое человеком: побеждает то, к чьей середине
   * курсор ближе. На середине планеты это планета, на самом спутнике - спутник,
   * и объяснять тут нечего.
   */
  if (dot && (!disc || (dotDepth < discDepth && dot.distancePx < disc.distancePx))) return dot;
  return disc ?? dot;
}
