...
export interface SceneInputOptions {
  ...
  /** Начать перелёт к телу, выбранному зрителем: экскурсия на этом кончается. */
...
function bindPointer(options: SceneInputOptions): void {
  ...
  /*
   * Протаскивание пальцем, начатое во время экскурсии, не решает свою судьбу
   * сразу.
   *
   * Горизонтальное движение переводит экскурсию на соседнюю остановку, а всё
   * остальное обрывает. Отличить одно от другого можно только по движению,
   * поэтому касание лишь запоминает точку, а решение принимается на первом же
   * сдвиге сверх порога (или на отпускании - тогда это был щелчок).
   *
   * Мыши свайп не нужен: у неё под рукой клавиатура, и остановки листают
   * стрелки. Протаскивание мышью - это осмотр, и оно обрывает экскурсию сразу.
   */
  ...
  canvas.addEventListener('pointerdown', (event) => {
    ...
    if (tour.isActive) {
      // Пока экскурсия идёт, судьба касания решается по движению. Свайп нужен
      // и на перелёте между остановками, где орбитального режима ещё нет, -
      // поэтому проверка орбиты сюда не входит.
      //
      // Мышь сюда не допущена. С ней протаскивание, принятое за свайп,
      // уводило к следующей планете того, кто взялся повернуть текущую, а
      // короткое не делало ничего, пока экскурсия поворачивала тело сама.
      // Нажатие мышью обрывает экскурсию, и дальше это обычный осмотр.
      if (event.pointerType !== 'mouse' && event.button === 0 && !flight.isLocked) {
