...
export class Atmosphere {
  ...
  update(
    ...
    // множитель размеров раздувает оболочку масштабом меша, не трогая ни
    // геометрию, ни высоты, по которым считается плотность воздуха.
    // Поворот, обратный ориентации тела: им и камера, и Солнце переводятся
    // из координат сцены в систему координат планеты.
    ...
    uniforms.uSunIrradiance!.value = SOLAR_IRRADIANCE_SCALE / (distance * distance);
