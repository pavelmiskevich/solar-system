...
const SHOTS = [
  ...
  {
    file: 'comet.png',
    what: 'Комета Галлея в перигелии: пылевой хвост веером, ионный прямой',
    // Февраль 1986-го: прохождение перигелия. Смотреть надо от Солнца, а не
    // от самой кометы - ядро в одиннадцать километров и хвост в треть
    // астрономической единицы в один кадр не помещаются.
    date: '1986-02-08T00:00:00Z',
    // Камера стоит в стороне от луча «Солнце - комета», иначе хвост уходит в
    // блик звезды. Перигелий у Галлеи на долготе 307 градусов, камера взята
    // под прямым углом к нему и выше плоскости эклиптики.
    // 1.8 а.е. от Солнца, долгота 37°, выше эклиптики на 35°. Числа в
    // километрах, а не в единицах: place уезжает в браузер как текст, и
    // константы этого файла туда не попадают.
    place: (sim) => sim.lookAt([175230486, 154450464, -133973823], [0, 0, 0]),
  },
  {
    file: 'belt.png',
    what: 'Пояс астероидов: кольцо малых тел между Марсом и Юпитером',
    // Девять астрономических единиц сверху - единственная высота, с которой
    // пояс виден целиком и вместе с орбитами, между которыми он лежит. Ближе
    // кольцо уходит за край кадра, дальше тонет в звёздах.
    place: (sim) => sim.lookAt([0, 9 * 1.495978707e8, 0.02 * 1.495978707e8], [0, 0, 0]),
  },
  {
    file: 'system.png',
  ...
  {
    file: 'events.png',
    what: 'Ближайшие события: список посчитан по эфемеридам, а не записан в коде',
    place: (sim) => sim.travelTo('earth'),
    awaitTravel: true,
    // Первое B закрывает список тел, открытый предыдущим кадром; E открывает
    // события. Список считается от даты сцены, а она у съёмки постоянная, -
    // значит, и кадр повторяется в точности.
    press: ['KeyB', 'KeyE'],
  },
];
