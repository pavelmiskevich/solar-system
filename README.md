...
| RU / EN в правой колонке | язык интерфейса, см. «Язык» ниже |
| `H` | справка |
...
Вид сцены пишется в адрес страницы: дата, выбранное тело, положение камеры,
скорость времени и язык интерфейса. Ссылкой можно поделиться - «смотри, вот затмение» - и
...
На невысоком экране список и карточка делят то, что осталось под кнопками.
Первым уступает список, но не меньше трёх строк: следующее тело выбирают из
него же. Дальше сжимается карточка и прокручивается.
...
Интерфейс - на русском и английском. При первом входе язык берётся из
настроек браузера: русский, если первый из знакомых сцене языков русский,
иначе английский. Переключатель RU/EN стоит в правой колонке рядом со ссылкой
на GitHub; выбор запоминается в браузере и попадает в адрес страницы
параметром `lang`, так что ссылка открывается на том языке, на котором ею
поделились. Старшинство такое: язык в ссылке, потом выбранный руками, потом
язык браузера. Язык меняется на лету, без перезагрузки: справка, список тел,
HUD, карточка, события и подписи в сцене переписываются сразу.

Все слова интерфейса лежат в одном словаре - `src/i18n/ru.ts` и
`src/i18n/en.ts`, и тип английского выводится из русского: строку, которую
забыли перевести, ловит `tsc`, а русскую фразу, скопированную в английский
словарь, - юнит-тест. Строки с числами и падежами - функции, а не склейка:
«спутник Марса» и «moon of Mars» устроены по-разному. Имена звёзд и созвездий
генератор неба пишет сразу на двух языках; английские имена звёзд - собственные
имена из каталога HYG, созвездия - латинские имена МАС.

Комментарии и документация остаются на русском, и это выбор, а не недоделка:
причины решений в комментариях - текст для тех, кто правит код, а не для
зрителя.
...
src/
  ...
  ui/         подписи, списки тел, готовых видов и ближайших событий,
              ...
              снимок кадра, попадание кликом, переключатель языка
  i18n/       словари интерфейса на русском и английском, выбор языка
...
и в [задачах](https://github.com/pavelmiskevich/solar-system/issues): затмения с
разных планет, режим поздравления на созвездиях.
